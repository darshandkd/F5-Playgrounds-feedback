...
| **[XC Distributed Cloud](https://playgrounds.darshandkd.com/xc/)** | **F5 Distributed Cloud — now live and growing.** Interactive scenarios across **Multi-Cloud Networking** (App Connect — "connect apps, not networks"), **WAAP** (WAF attack signatures, API discovery & schema validation, L7 DDoS, web-app scanning), and **Bot Defense & Fraud** (adaptive bot defense, per-request signal detection, client-side / Magecart defense, authentication intelligence) — plus platform **Fundamentals** (Regional Edges, Customer Edges, the F5 Global Network). Modelled on the real XC mental model; more scenarios land regularly. |
